...
describe('Testing The User Controller', function () {
  ...
  beforeEach(inject(function ($rootScope, $controller, _UserService_, _FormSubmissionUtilService_, _RESULT_SIZE_,_CompanyService_) {
  ...
  it('Should Call CompanyService.query and set the list of companies on the scope',function(){
    var companies = {content:[1,2,3,4,5,6,7,8,9,0]};
    var params = {skip:0,limit:RESULT_SIZE};
    spyOn(CompanyService,'get').andReturn(companies);
    expect($scope.init).toBeDefined();
    $scope.init();

    expect(CompanyService.get).toHaveBeenCalledWith(params,jasmine.any(Function));
    expect($scope.companies).toEqual(companies.content);
  });
